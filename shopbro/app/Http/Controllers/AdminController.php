...
class AdminController extends Controller
{
    ...
    public function show_dashboard(Request $request)
    {
        ...
        // ->with(compact('visitors_total','visitor_count'));
    }
    ...
    public function callback_customer_facebook(){
     ...
     return redirect('/trang-chu')->with('message','Đăng nhập bằng tài khoản facebook
     <span style="color:red">'.$account_name->customer_email.'</span> thành công');
        }
        elseif($account==NULL){
        ...
        return redirect('/trang-chu')->with('message','Đăng nhập bằng tài khoản facebook
        <span style="color:red">'.$account_new->customer_email.'</span> thành công');
    }
    ...
    public function callback_customer_google(){
     ...
     return redirect('/trang-chu')->with('message','Đăng nhập bằng tài khoản google
         <span style="color:red">'.$account_name->customer_email.'</span> thành công');
    }
    public function findOrCreateCustomer($users,$provider){
        $authUser= SocialCustomers::where('provider_user_id',$users->id)
        ->where('provider_user_email',$users->email)->first();
        if($authUser){
            return $authUser;
        }else{
    ...
    public function dashboard(REQUEST $request)
    {
        ...
        if ($result) {
            ...
            Session::put('message', 'Tên tài khoản hoặc mật khẩu sai.Vui lòng nhập lại.');
    ...
    public function filter_by_date(Request $request)
    {
        ...
        if(empty($from_date) || empty($to_date)){
            $get = DB::table('tbl_order')->orderby('updated_at','desc')->get();
        }else{
            $get = DB::table('tbl_order')->whereBetween('updated_at', [$from_date, $to_date])->orderby('updated_at','desc') ->get();
        }
    ...
    public function dashboard_filter(Request $request)
    {
        ...
        if ($data['dashboard_value'=='7ngay']) {
            ...
            $get=Order::whereBetween('created_at', [$dauthangtruoc,$cuoi_thang_truoc])->orderBy('created_at', 'ASC')->get();
    ...
    // );
        ...
        public function days_order(request $request){
            $sub60ngay=Carbon::now('Asia/Ho_Chi_Minh')->subdays(60)->toDateString();
            $now=Carbon::now('Asia/Ho_Chi_Minh')->toDateString();

            $get=Order::whereBetween('created_at', [$sub60ngay,$now])->orderBy('created_at', 'ASC')->get();
            foreach ($get as $key => $val) {
                $chart_data[]=array(
                    'period'=>$val->order_date,
                    'order'=>$val->total_order,
